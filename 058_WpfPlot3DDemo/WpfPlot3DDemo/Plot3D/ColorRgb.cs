...
namespace WpfPlot3DDemo.Plot3D
{
	...
	/// Author      : BigWang
	/// Created Time: 2024/8/22 23:54:09
	/// Description :
	/// ----------------------------------------------------------------
	/// Version      Modified Time              Modified By     Modified Content
	/// V1.0.0.0     2024/8/22 23:54:09                     BigWang         首次编写         
	///
    [Serializable, StructLayout(LayoutKind.Sequential), DebuggerDisplay("\\{ARGB = ({A}, {R}, {G}, {B})\\}")]
    public struct ColorRgb : IEquatable<Color>
    {
        [DebuggerStepThrough]
        internal static void Checkdouble(double value, string name) => Checkdouble(value, name, 0.0, 1.0);
        ...
        internal static void Checkdouble(double value, string name, double min, double max)
        {
            ...
            {
                throw new ArgumentException(string.Format("Name = {0}, value = {1}, min = {2}, max = {3}", name, value, min, max));
            }
        }
        ...
        public override bool Equals(object obj) => (obj is Color color && ToColor() == color) || (obj is ColorRgb rgb && ToColor() == rgb);

        public static bool operator ==(ColorRgb c1, ColorRgb c2) => c1.A == c2.A && c1.R == c2.R && c1.G == c2.G && c1.B == c2.B;

        public static bool operator !=(ColorRgb c1, ColorRgb c2) => !(c1 == c2);

        public bool Equals(Color other) => ToColor() == other;

        public override int GetHashCode() => A.GetHashCode() ^ R.GetHashCode() ^ G.GetHashCode() ^ B.GetHashCode();

        public static ColorRgb FromColor(Color color) => new ColorRgb(color.A / 255.0, color.R / 255.0, color.G / 255.0, color.B / 255.0);

        public static ColorRgb FromColor(double alpha, Color baseColor) => new ColorRgb(alpha, baseColor.R / 255.0, baseColor.G / 255.0, baseColor.B / 255.0);

        public static implicit operator ColorRgb(Color color) => FromColor(color);
        ...
        public Color ToColor() => Color.FromArgb((int)(255.0 * A + .5), (int)(255.0 * R + .5), (int)(255.0 * G + .5), (int)(255.0 * B + .5));

        public static explicit operator Color(ColorRgb rgb) => rgb.ToColor();
        ...
        public static implicit operator ColorHsv(ColorRgb rgb) => rgb.ToColorHsv();
        ...
        public static implicit operator ColorRgb(ColorHsl hsl) => FromColor(hsl);
        ...
        public static implicit operator ColorHsl(ColorRgb rgb) => rgb.ToColorHsl();
    }
}